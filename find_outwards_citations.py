        ...
        if paper["venue_id"] == THE_CONFERENCE:
            for citation_id in paper["references"]:
                if citation_id in older_papers.keys():
                    citation_data[year][CONFERENCES_MAP[older_papers[citation_id]["venue_id"]]] = (
                        citation_data[year].get(CONFERENCES_MAP[older_papers[citation_id]["venue_id"]], 0) + 1
                    )
                else:
                    citation_data[year]["OTHER"] = citation_data[year].get("OTHER", 0) + 1
