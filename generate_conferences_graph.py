...
def main(save_adj: bool = False,
        ...
        for cnt, line in tqdm(enumerate(f), total=DBLP_SIZE):
            ...
            try:
                if parsed_json['venue']['id'] == CONFERENCE_ID:
                    ...
                    conference_papers[int(parsed_json['year'])].append(
    ...
    for year in sorted(conference_papers.keys()):
        ...
        # Add papers to be referenced
        older_papers.extend([paper['id'] for paper in year_conference_papers])

        print("I'm in year {} coloring {}".format(
            str(year), year_colors[year]))
    ...
    if save_adj:
        with open('./dblp_arnet/{}_graph.json'.format(CONFERENCE_NAME), 'w') as f:  # nopep8
