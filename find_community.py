...
def run_girvan_newman(G: nx.Graph):
    ...
    def highest_betweenness(G):
        edges = list(G.edges)
        betweenness = {(u, v): (G.nodes[u].get(
            'betweenness', 0) + G.nodes[v].get('betweenness', 0)) / 2 for (u, v) in edges}
        return max(betweenness, key=lambda x: x[1])

    tqdm.write('Running girvan_newman method')
    centrality = community.centrality.girvan_newman(
        G, most_valuable_edge=highest_betweenness)
...
def save_communities(G, communities, filename: str, extra_filename: str = ''):
    write_filename = SAVE_FOLDER + \
        filename.split('/')[-1] + \
        ".{}_{}.json".format('communities', extra_filename)
...
def find_community(search_path: str):
    """
        path: str = Name of a GML file which we want to find the communities
